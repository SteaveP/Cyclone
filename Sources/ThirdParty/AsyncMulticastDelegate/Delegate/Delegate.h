...
public:
	...
	virtual bool operator==(const DelegateBase& rhs) const 	{
		const DelegateMember0<TClass, RetType>* derivedRhs = static_cast<const DelegateMember0<TClass, RetType>*>(&rhs);
		return derivedRhs &&
...
public:
	...
	virtual bool operator==(const DelegateBase& rhs) const 	{ //#todo_hack here was dynamic_cast. need to fix smthing?
		const DelegateMember1<TClass, Param1, RetType>* derivedRhs = static_cast<const DelegateMember1<TClass, Param1, RetType>*>(&rhs);
		return derivedRhs &&
			m_func == derivedRhs->m_func && 
			m_object == derivedRhs->m_object; }

...
LRESULT CALLBACK CycloneWndProc(HWND hWnd, uint32 Message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SIZE:
        if (Window)
        {
            // #todo_window #todo_vk_swapchain disable rendering if in background
...
//         if (Window)
//         {
//             // #todo_window promopt for conformation
//         }
//         break;
    case WM_DESTROY:
        PostQuitMessage(0);
...
WindowWinApi::WindowWinApi(WindowWinApi&& Other) noexcept : IWindow(MoveTemp(Other))
{
    ...
    std::swap(m_WindowActive, Other.m_WindowActive);
    std::swap(m_CenterCursor, Other.m_CenterCursor);
    std::swap(m_ShowCursor, Other.m_ShowCursor);
    std::swap(m_PendingCursorVisibility, Other.m_PendingCursorVisibility);
    std::swap(m_ScaleWindowWithRespectToDPI, Other.m_ScaleWindowWithRespectToDPI);
    std::swap(m_App, Other.m_App);
    std::swap(m_Name, Other.m_Name);
}
WindowWinApi& WindowWinApi::operator=(WindowWinApi&& Other) noexcept
{
    if (this != &Other)
    {
        IWindow::operator=(MoveTemp(Other));
        std::swap(m_hWnd, Other.m_hWnd);
        std::swap(m_hInstance, Other.m_hInstance);
        std::swap(m_Width, Other.m_Width);
        std::swap(m_Height, Other.m_Height);
...
WindowWinApi::~WindowWinApi()
{
    ...
    CASSERT(m_hWnd == nullptr);
}

C_STATUS WindowWinApi::Init(const WindowDesc* Desc)
{
    const CPlatformDataWin* WinApiDesc = reinterpret_cast<const CPlatformDataWin*>(Desc->PlatformDataPtr);

    m_App = Desc->App;
    m_Width = Desc->Size.X;
    m_Height = Desc->Size.Y;
    m_hInstance = WinApiDesc->hInstance;
    m_Name = Desc->Name;

    int CmdShowFlag = WinApiDesc->nCmdShow;
...
void WindowWinApi::DeInitImpl()
{
    // #todo_window nothing to do?
    m_hWnd = nullptr;
}
...
void WindowWinApi::OnResize(unsigned int NewWidth, unsigned int NewHeight)
{
    ...
    {
        m_App->GetRenderer()->OnWindowResize(this);
    }
}
...
C_STATUS WindowWinApi::UpdateMessageQueue()
{
    ...
    // #todo_vk #todo_window what about multi windows? is this read messages from all windows and close all windows if need to close only one?
    // read all messages

...
#if PLATFORM_WIN64
//IMGUI_VULKAN_FUNC_DEF(vkCreateWin32SurfaceKHR)
static PFN_vkCreateWin32SurfaceKHR MyC;
#endif
...
static int ImGui_ImplVulkan_CreateVkSurface(ImGuiViewport* vp, ImU64 vk_inst, const void* vk_allocators, ImU64* out_vk_surface); // (Optional) For a Vulkan Renderer to call into Platform code (since the surface creation needs to tie them both).
...
#if PLATFORM_WIN64
    MyC = reinterpret_cast<PFN_vkCreateWin32SurfaceKHR>(loader_func("vkCreateWin32SurfaceKHR", user_data));
#endif
...
int ImGui_ImplVulkan_CreateVkSurface(ImGuiViewport* vp, ImU64 vk_inst, const void* vk_allocators, ImU64* out_vk_surface)
{
    VkInstance InstanceVk = (VkInstance)vk_inst;
    ...
    VkResult Result = MyC(InstanceVk, &CreateInfo, nullptr, &SurfaceVk);
...
#error Unsupported platform
    ...
    *out_vk_surface = (ImU64)SurfaceVk;
